...
class NLtoSQL:
    ...
    def apply(self, question):
        ...
        Returns:
            tuple: A tuple containing the SQL code and retrieved data, or (None, None) if an error occurs.
        """
        ...
        for i in range(self.__tries):
            try:
                ...
                tables_info = [{tables[i].strip("' "): self.__tables_dict[tables[i].strip("' ")]} for i in range(len(tables))]
                break
        ...
        for i in range(self.__tries):
            try:
                ...
                SQLcode = get_tags_info(coder_response, tag="code")
                SQLcode = SQLcode.strip('][ ').split('##D##')
                data_tables_names = get_tags_info(coder_response, tag="tablename").strip('][ ').split(',')
                data_tables = []
                cursor = self.__cur_connection.cursor()
                j = 0
                while j < len(SQLcode):
                    cursor.execute(SQLcode[j])
                    ...
                    data_tables.append((data_tables_names[j].strip(), pd.DataFrame(final_result_data, columns=headers)))
                    j += 1
                nice_print("Missions succeeded.\n")
                nice_print(f"""Question asked: "{question}"\n""")
                nice_print("Data extracted:\n" + columns_reasoning + '\n')
                return SQLcode, data_tables
            except Exception as e:
                prompt = NLtoSQL.FIXER2.format(MAIN_PROMPT=main_prompt, RESPONSE=coder_response, I=j, E=e)
        return None, None
